...
srun -N 1 --gres=gpu:1 -u --pty -p m40t4 \
  python test.py --net=BN-Inception \
  --dim=512 --width=224 --batch_size=256 --data=car \
  --data_root=/home/nfs/admin0/yaoguang/code/MetricLearning/Object-retrieval-others/DataSet \
  --resume=/home/nfs/admin0/yaoguang/code/MetricLearning/Object-retrieval-others/log/Weight/car/checkpoints/car_ms_bs256_MultiRepeat_fulltest \
  --testAllAngle=True
...
# OPTIM=SGD
# LR=5e-2
# LOG_NAME=car_ms_bs256_MultiRepeat_fulltest
...
# # if [ ! -n "$1" ] ;then
# echo "Begin Training!"
# srun -N 1 --gres=gpu:t4:1 -u --pty -p m40t4 python train.py --net ${NET} \
# --data $DATA \
# --optim $OPTIM \
# --momentum ${Momentum} \
