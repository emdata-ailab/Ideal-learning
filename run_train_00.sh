#!/usr/bin/env bash
DATA=car
DATA_ROOT=/home/jianfei.gao/code/Deep_metric-master/DataSet
...
SAVE_DIR=${CHECKPOINTS}/${LOSS}/${DATA}/${NET}-DIM-${DIM}-lr${LR}-ratio-${RATIO}-BatchSize-${BatchSize}-S-${BETA}-M-${ALPHA}
...
--save_step 50 \
--ratio ${RATIO} \
| tee -a log/$LOSS/$DATA/${NET}-DIM-$DIM-Batchsize-${BatchSize}-ratio-${RATIO}-lr-$LR${POOL_FEATURE:+'-pool_feature'}-S-${BETA}-M-${ALPHA}.txt
...
for i in $Model_LIST; do
    ...
    | tee -a result/$LOSS/$DATA/${NET}-DIM-$DIM-Batchsize-${BatchSize}-ratio-${RATIO}-lr-$LR${POOL_FEATURE:+'-pool_feature'}-S-${BETA}-M-${ALPHA}.txt
