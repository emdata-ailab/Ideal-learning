...
class LimitHingeLoss(nn.Module):
    def __init__(self, alpha=10, beta=2, margin=0.5, hard_mining=None, dim = 2048, **kwargs):
        super(LimitHingeLoss, self).__init__()
        self.margin = margin
        self.alpha = alpha #s
        self.beta = beta #m
        self.dim = dim
        self.hard_mining = hard_mining
        self.kernel = nn.Parameter(torch.Tensor(98,self.dim), requires_grad=True)
    ...
    def forward(self, inputs, targets):
       # print('%.8f' % torch.sum(self.kernel))
        ...
        print('%.8f' % ((torch.sum(proxy_sim) - 98.0) / (98.0 * 97))  )        
        
        proxy_neg = torch.mm(kernel_norm[targets], torch.t(kernel_norm))
        ...
        output[index] = output[index] - self.beta # only change the correct predicted output
        ...
        loss_mask = (torch.max(output,dim=1)[0]-margin - 1e-6)>0
